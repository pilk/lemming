...
public class StateCollection : MonoBehaviour
{
    ...
    //public GameObject m_default = null;

    //[GeneratedDropDown("GetAvailableStateNames")]
    ...
    {
        if (force || m_currentState == null || (m_currentState.stateName.Equals(state, System.StringComparison.Ordinal) == false))
        {
            m_currentState = null;
            for (int i = m_stateEntries.Count - 1; i >= 0; --i)
            {
                bool isTarget = false;
				if (m_stateEntries[i] != null && m_stateEntries[i].stateName != null)
				{
					isTarget = m_stateEntries[i].stateName.Equals(state, System.StringComparison.Ordinal);
				}
