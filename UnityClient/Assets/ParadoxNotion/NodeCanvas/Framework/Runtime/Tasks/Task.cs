	...
	abstract public partial class Task {
		...
		protected Component agent{
			get
			{
				...
				if (agentIsOverride){
					return (Component)overrideAgent.value;
				}

				if ( ownerAgent != null && agentType != null && ( agentType.RTIsSubclassOf(typeof(Component)) || agentType.RTIsInterface() ) ){
					return ownerAgent.GetComponent(agentType);
				}
		...
		bool Initialize(Component newAgent, Type newType){
			...
			if (!agentIsOverride && newAgent != null && newType != null && ( newType.RTIsSubclassOf(typeof(Component)) || newType.RTIsInterface() ) ){
				newAgent = newAgent.GetComponent(newType);
			}
